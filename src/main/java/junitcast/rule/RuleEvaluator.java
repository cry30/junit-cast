...
public class RuleEvaluator<T> {
	...
	private static final String OPERATORS = Operator.Not + String.valueOf(Operator.Or) + Operator.And; // NOPMD: This is
																										// not a String.
	 ...
	 *
	 * @param pExpression <code>String</code> input expression (logical expression
	 *                    formula)
	 * @since 0.3.0
	 */
	public void parse(final String pExpression)
	{
		...
		final StringTokenizer stringTokenizer = new StringTokenizer(expression, RuleEvaluator.OPERATORS + "()", true);
	...
	private void doShuntInternal(final String token)
	{
		if (getLogicHelper().isOpenBracket(token)) {
			...
			while (!this.stackOperations.empty()
					&& !getLogicHelper().isOpenBracket(this.stackOperations.lastElement().trim())) {
				this.stackRPN.push(this.stackOperations.pop());
			}
			this.stackOperations.pop();
		} else if (isOperator(token)) {
			while (!this.stackOperations.empty() && isOperator(this.stackOperations.lastElement().trim())
					&& getPrecedence(token.charAt(0)) <= getPrecedence(
	 ...
	 */
	public Boolean evaluate(final List<T> scenario, final Map<String, ElementConverter> ruleTokenConvert)
	{
		Boolean retval;
	 ...
	 */
	private Boolean evaluateMultiRpn(final List<T> scenario, final Map<String, ElementConverter> ruleTokenConvert)
	{
		/* clean answer stack */
		...
		final Stack<String> stackRPNClone = (Stack<String>) this.stackRPN.clone();
		...
		while (!stackRPNClone.empty()) {
			...
			if (isOperator(token)) {
				if (Operator.Not.toString().equals(token)) {
					...
					evaluateMulti(scenario, ruleTokenConvert, Operator.fromChar(token.charAt(0)));
				}
			} else {
	 ...
	 */
	private void evaluateMulti(final List<T> scenario, final Map<String, ElementConverter> ruleTokenConvert,
			final Operator operator)
	{
		final ElementConverter defaultConverter = defaultConvertMap.get(scenario.get(0).getClass());
		final Object[] leftArr = getNextValue(ruleTokenConvert, defaultConverter);
		final Object[] rightArr = getNextValue(ruleTokenConvert, defaultConverter);

		Method method;
		try {
			method = LogicHelper.class.getDeclaredMethod("performLogical" + operator.name(),
					new Class[] { List.class, Integer.TYPE, Integer.TYPE, Object.class, Object.class });

			final String answer = (String) method.invoke(getLogicHelper(),
					new Object[] { scenario, (Integer) leftArr[0], (Integer) rightArr[0], leftArr[1], rightArr[1] });
	...
	private void evaluateMultiNot(final List<T> scenario)
	{
		...
		if (LogicHelper.TRUE.equals(left)) {
			...
			if (subscript > -1) {
				final ElementConverter defaultConverter = defaultConvertMap.get(scenario.get(0).getClass());
				answer = String.valueOf(!scenario.get(subscript)
						.equals(defaultConverter.convert(left.substring(0, left.indexOf('[')))));
			} else {
	...
	private Boolean evaluateOneRpn(final List<T> scenario)
	{
		...
		final ElementConverter defaultConverter = defaultConvertMap.get(scenario.get(0).getClass());
		if (subscript > -1) {
			retval = scenario.get(subscript).equals(defaultConverter.convert(single.substring(0, single.indexOf('['))));
		} else {
			retval = scenario.contains(defaultConverter.convert(single));
		}
		return retval;
	}
	 ...
	 */
	Object[] getNextValue(final Map<String, ElementConverter> ruleTokenConvert, final ElementConverter defaultConverter)
	{
		...
		if (!LogicHelper.TRUE.equals(value) && !LogicHelper.FALSE.equals(value)) {
			...
			if (subscript > -1) {
				final ElementConverter converter = this.converterList.get(subscript);
				value = converter.convert(valueStr.substring(0, valueStr.indexOf('[')));
			} else {
				if (ruleTokenConvert == null || ruleTokenConvert.get(valueStr) == null) {
	...
	int getSubscript(final String string)
	{
		...
		if (matcher.find()) {
			final String indexStr = string.substring(string.indexOf('[') + 1, string.indexOf(']'));
